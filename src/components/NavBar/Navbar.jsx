import React from 'react'
import './Navbar.css';
import logo from '../images/ballers_logo.png';
import 'https://kit.fontawesome.com/b56885f075.js';
import { useState } from 'react';

function Navbar() {
    const [isOpen, setIsOpen] = useState(false);

    const handleMenuClick = () => {
        setIsOpen(!isOpen);
    };

    const handleOverlayClick = () => {
        setIsOpen(false);
    };
    return (

        <nav className={` ${isOpen ? 'open' : ''}`}>
            <div className="logo">

                <div className='containNav'>

                    <div className="boxNav">
                        <i class="fa-solid fa-bars" id='none' onClick={handleMenuClick}></i>
                        <img src={logo} alt="Ballers Logo" />
                    </div>
                    <div className="boxNav display">
                        <ul class="navbar-bar">
                            <li class="bar-item"><a class="bar-link" href="#">Self assessment</a></li>
                            <li class="bar-item"><a class="bar-link" href="#">About Salimeen</a></li>
                            <li class="bar-item"><a class="bar-link" href="#">Our team</a></li>
                            <li class="bar-item">|</li>
                            <li class="bar-item">
                                <label htmlFor=""><i class="fa-solid fa-earth-americas"></i></label>
                                <select className="selectNav" name="" id="">
                                    <option value="" selected>Language</option>
                                    <option value="">test</option>
                                    <option value="">test</option>
                                </select>
                            </li>
                        </ul>
                    </div>
                    <div className="boxNav ">
                        <button className='btn-Nav'>login</button>
                    </div>

                </div>
            </div>
            <div className="sidebar">
                <div className="logo">
                    <i class="fa-solid fa-bars" onClick={handleOverlayClick}></i>
                    <img src={logo} alt="Ballers Logo" id='sidebar-logo' />
                </div>
                <div className="sidebar-content">
                    <ul className="lists">
                        <li className="list">
                            <a href="#" className="nav-link">
                                <span className="link">Self assessment</span>
                            </a>
                        </li>
                        <li className="list">
                            <a href="#" className="nav-link">
                                <span className="link">About Salimeen</span>
                            </a>
                        </li>
                        <li className="list">
                            <a href="#" className="nav-link">
                                <span className="link">Our team</span>
                            </a>
                        </li>
                        <li className="list">
                            <label htmlFor=""><i class="fa-solid fa-earth-americas"></i></label>
                            <select className="selectNav" name="" id="">
                                <option value="" selected>Language</option>
                                <option value="">test</option>
                                <option value="">test</option>
                            </select>
                        </li>
                        
                    </ul>
                    <div className="bottom-cotent">
                        <li className="list">
                            <a href="#" className="nav-link">
                                <i className="bx bx-cog icon" />
                                <span className="link">Settings</span>
                            </a>
                        </li>
                    </div>
                </div>
            </div>
        </nav>

    )
}

export default Navbar